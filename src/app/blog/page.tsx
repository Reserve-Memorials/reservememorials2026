...
export default async function BlogIndexPage() {
  const posts = await getWpPosts({ perPage: 12, page: 1 });
      ...
      <section className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/60 p-8 shadow-sm backdrop-blur sm:p-12">
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <p className="max-w-2xl text-pretty text-base text-muted-foreground sm:text-lg">
            Helpful articles on memorial design, materials, timelines, and planning—written to be
            clear, kind, and practical.
          </p>
        </div>
      </section>
